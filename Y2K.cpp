...
int main()
{
   //signature
    ...
    // declare variables
    int numRuns, sequence[3], currentNum, pattern, time, count;
    ...
    for(int j = 0; j < numRuns; j++)
    {
        ...
        // determines if any number in the sequence ends in a 2
        currentNum = sequence[0];
        do
        {
            if(currentNum % 10 == 2 or currentNum % 10 == -2)
            {
                count++;
            }
            time++;
        ...
        // prints the answer
        cout << "\nThe amount of times an integer ended in a 2 was " << count << "." << endl;
    }
